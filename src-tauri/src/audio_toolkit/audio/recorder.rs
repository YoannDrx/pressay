...
impl AudioRecorder {
    ...
    ///
    /// CPAL can either terminate the callback path or report a stream error
    /// while leaving the channel and worker alive. Both cases must reopen on
    /// the next recording request.
