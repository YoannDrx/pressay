...
fn build_storekit_bridge() {
    ...
    const SWIFT_FILE: &str = "swift/storekit.swift";
    const BRIDGE_HEADER: &str = "swift/storekit_bridge.h";
    println!("cargo:rerun-if-changed={SWIFT_FILE}");
    println!("cargo:rerun-if-changed={BRIDGE_HEADER}");

    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR not set"));
    let object_path = out_dir.join("pressay_storekit.o");
    let static_lib_path = out_dir.join("libpressay_storekit.a");
    ...
    let status = Command::new(&swiftc_path)
        .args([
            ...
            object_path
                .to_str()
                .expect("StoreKit object path is invalid"),
        ])
        .status()
        .expect("Failed to invoke swiftc for StoreKit bridge");
    if !status.success() {
    ...
    let status = Command::new("libtool")
        .args([
            ...
            static_lib_path
                .to_str()
                .expect("StoreKit static library path is invalid"),
            object_path
                .to_str()
                .expect("StoreKit object path is invalid"),
        ])
        .status()
        .expect("Failed to create StoreKit static library");
    if !status.success() {
        panic!("libtool failed for StoreKit bridge");
    }

    println!("cargo:rustc-link-search=native={}", out_dir.display());
    println!("cargo:rustc-link-lib=static=pressay_storekit");
    println!(
